...
# weather
from chatbot2 import byHour
from chatbot2 import y
from chatbot5 import data

a="The highest price is {}, What else do you want to know?"
b="The lowest price is {}, What else do you want to know?"
...
class Chatbot:
    ...
    def respond(self,state, message):
        ...
        if self.get_intent(message) == 'highest':
            response = a.format(data['high'])
        if self.get_intent(message) == 'lowest':
            response = b.format(data['low'])
        if self.get_intent(message) == 'finish':
            response = policy_rules[(state, self.get_intent(message))][1]

        if self.get_intent(message) == 'query_weather':
            response = policy_rules[(state, self.get_intent(message))][1]

        if self.get_intent(message) == 'summary':
            response = policy_rules[(state, self.get_intent(message))][1].format(byHour.summary)

        if self.get_intent(message) == 'icon':
            response = policy_rules[(state, self.get_intent(message))][1].format(byHour.icon)

        if self.get_intent(message) == '48_temperature':
            response = policy_rules[(state, self.get_intent(message))][1].format(y)

        if self.get_intent(message) == 'weather_plot':
            response = policy_rules[(state, self.get_intent(message))][1]
    ...
    def get_current_price(self,company):
        ...
        stock = Stock(company, token="pk_77f74e3b59e54d43b48e72de351caaf0")
        prices = str(stock.get_price())
        return prices

    def get_ttmEPS(self,company):
        ttmEPS = Stock(company, token="pk_77f74e3b59e54d43b48e72de351caaf0").get_key_stats()['ttmEPS']
        return ttmEPS

    def get_news(self,company):
        news = Stock(company, token="pk_77f74e3b59e54d43b48e72de351caaf0").get_news()
        for i in news:
    ...
    def generate_figure(self,message,):
        ...
        if len(comprehended_data['entities']) <= 3:
            ...
            his_data = get_historical_data(required_company, start_time, end_time, output_format='pandas', token="pk_77f74e3b59e54d43b48e72de351caaf0")
